...
int main(int argc, char** argv) {
	...
	if (n != 0 && n != 1) {
		printf(
				"Please enter 0 or 1 as the third parameter. 0 if you would not like to follow Symbolic Links, 1 if you would.\n");
		return 0;
	}
	char userinput[32];
	if (access(argv[1], F_OK) != -1) {
		printf(
				"The file already exists.  Would you like to overwrite? (Yes or No)\n");
		scanf("%s", userinput);
